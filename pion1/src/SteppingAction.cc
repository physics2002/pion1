...
void SteppingAction::UserSteppingAction( const G4Step* theStep ) {
  ...
  if ( theStep->GetTrack()->GetParentID() == 0  &&
       ...
       theStep->GetPostStepPoint()->GetPosition().z() >= 0.0*m  &&
       theStep->GetPostStepPoint()->GetPosition().z() <= 100.*m  &&
...
//    man->FillNtupleDColumn(0, zPos/1000);
//    man->AddNtupleRow(0);
    ...
    const G4double E = theStep->GetPostStepPoint()->GetKineticEnergy();
    ...
    const G4double TEMUR =
      57.29*std::acos( theStep->GetPostStepPoint()->GetMomentumDirection().z() );
    A = TEMUR;
    
    const G4double delta_ekin = fPrimaryParticleInitialKineticEnergy - E;
    //G4cout << std::setprecision(6)
    ...
    fMeanDeltaAngle_primaryDecay += TEMUR;
    ...
    fMeanEkin_primaryDecay += E;
    fMinEkin_primaryDecay = std::min( fMinEkin_primaryDecay, E );
    fMaxEkin_primaryDecay = std::max( fMaxEkin_primaryDecay, E );
  ...
  if ( theStep->GetTrack()->GetParentID() == 1  &&
       theStep->GetTrack()->GetTrackID() == 3 &&
       theStep->GetPreStepPoint()->GetPosition().z() >= 0.0*m  &&
       theStep->GetPreStepPoint()->GetPosition().z() <= 100.*m &&
       theStep->GetTrack()->GetDefinition()->GetPDGEncoding() == 13 &&
       theStep->GetTrack()->GetCurrentStepNumber() == 1 &&
       theStep->GetPostStepPoint()->GetProcessDefinedStep() != nullptr  &&
//       sqrt((theStep->GetPostStepPoint()->GetPosition().x())*(theStep->GetPostStepPoint()->GetPosition().x()) + (theStep->GetPostStepPoint()->GetPosition().y())*(theStep->GetPostStepPoint()->GetPosition().y()))<=1.*m &&
       theStep->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName().find( "Transportation" )
       != std::string::npos ) 
    {
       ...
       const G4double TEMUR = 57.29*std::acos( theStep->GetPostStepPoint()->GetMomentumDirection().z() );
